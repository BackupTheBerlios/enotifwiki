...
<dl class="setup">
	<dd>
		<label class='column'>WYSIWYG Editor</label>
		<div>Select one:</div>

		<ul class="plain">
		<li><?php aField( $conf, "Editor", "enabled", "radio", "editor_enabled" ); ?></li>
		<li><?php aField( $conf, "Editor", "disabled", "radio", "editor_disabled" ); ?></li>
		</ul>
	</dd>
	<dt>
		<p>Use this to enable WYSIWYG (what-you-see-is-what-you-get) editing with FCKeditor for pages,
		where a token __USE_EDITOR__ is present.</p>
	</dt>
	</dl>
...
function writeLocalSettings( $conf ) {
	...
	if ( $conf->Editor == 'editor_enabled' ) {
		$editor = "## using an external WYSIWYG Editor: FCKeditor
...
##
\$wgUseEditor = false; # default: do not use editor, but wiki syntax
\$wgEditorDir = \"fckeditor\"; # path to the editor suite
\$wgEditorToken = \"__USE_EDITOR__\"; # magic word. When found, activate editor
\$wgEditorToolbarSet = \"Wiki\"; # FCKeditor uses this toolbar
