 ...
 function renderRss( $input ) {
    ...
    if (isset($args["version"])) $DisplayVersion = True; else $DisplayVersion = False;   
	...
	if ( isset($item['description']) ) {$description = True; break;}
    }
    ...
    $VersionText = "";
    if ($VersionNote) $VersionText = $VersionNote;
    if ($VersionLink and $VersionNo)  $VersionText = $VersionText. " - <a href=\"$VersionLink\" title=\"$VersionLink\">version $VersionNo</a>.";
    if (!$VersionLink and $VersionNo) $VersionText = $VersionText. " - version $VersionNo.";
	if ($DisplayVersion and $VersionText) $output = $output . "<div style=\"background-color: #E0E0E0; font-weight: bold; font-size: 60%; padding: 0em 0.5em 0em 0.5em;\" >$VersionText</div>";
