...
class User {
	...
	function loadFromSession() {
		...
		if ( isset( $_SESSION['wsToken'] ) ) {
			...
			return $wgAuth->initUser(); # Can't log in from session
		}
	...
	function loadFromDatabase() {
		...
		$s = $dbr->selectRow( 'user', array( 'user_name','user_password','user_newpassword','user_email',
		  'user_email_authenticated', 'user_enotif_newpages',
		...
		if ( $s !== false ) {
			...
			$this->mEnotifNewPages = $s->user_enotif_newpages;
	...
	function getEnotifNewPages() {
		$this->loadFromDatabase();
		return $this->mEnotifNewPages;
	}

	function setEnotifNewPages( $val ) {
		$this->loadFromDatabase();
		$this->mEnotifNewPages = $val;
	}
	...
	/**
	 * Check if a user is bureaucrat, developer, sysop
	 */
	function isBureaucratOrDeveloperOrSysop() {
		$this->loadFromDatabase();
		if ( 0 == $this->mId ) { return false; }
		return ( ( $this->isSysop() ) || ( $this->isDeveloper() ) || ( $this->isBureaucrat() ) );
	}
	...
	/**
	 * Check the user's notification timestamp for the given title and Namespace
	 *
	 * returns:	status of pending notification 		false, true
	 * out: 	lvr 					id of last visited revsion
	 */
	function checkNotificationPendingForArticleOrTalk( $title, $Ns, &$lvr ) {
		$userid = $this->getId();
		if ($userid==0)
			return;

		$dbr =& wfGetDB( DB_MASTER );
		$s = $dbr->selectRow( 'watchlist',
				array(
		...
		if ( $s === false ) {
			...
			$lvr = $s->wl_lastvisitedrevision;
			return ($s->wl_notificationtimestamp != wfTimestampOrNull() );
		}
	}
		...
		$dbw->update( 'user',
			array( /* SET */
				...
				'user_real_name' => $this->mRealName,
		 		...
		 		'user_enotif_newpages' => $this->mEnotifNewPages,
	...
	/**
	 * Checks if a user with the "real name" exists, returns the ID
         * The field "user_real_name" is used by Auto-Login / Auto-Account-Creation
	 * for storing the unique user login id as detected (can be hostname, for example)
	 */
	function idForRealName() {
		$fname = 'User::idForRealName';

		$gotid = 0;
		$s = trim( $this->mRealName );
		if ( 0 == strcmp( '', $s ) ) return 0;

		$dbr =& wfGetDB( DB_SLAVE );
		$id = $dbr->selectField( 'user', 'user_id', array( 'user_real_name' => $s ), $fname );
		if ( $id === false ) {
			$id = 0;
		}
		return $id;
	...
	function checkPassword( $password ) {
		...
		if ( 0 == strcmp( $ep, $this->mPassword ) ) {
			...
			/* $this->mNewpassword = ''; commenting this line fixes bugzilla 2126 */
			$this->saveSettings();
			return true;
