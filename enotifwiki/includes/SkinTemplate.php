		...
		*/
		...
		if ( $wgShowNewtalkForUserOrUserTalkPage ) {
				...
				if ($wgUser->checkNotificationPendingForArticleOrTalk( $usertitle, NS_USER_TALK, $lvr )) {
					...
					$newmsg_usertalklvrlink = $this->makeKnownLink( $ns_usertalk . ':' . $this->username, wfMsg( 'diff-to-lvr' ),
				...
				if ($wgUser->checkNotificationPendingForArticleOrTalk( $usertitle, NS_USER, $lvr )) {
					...
					$newmsg_userlvrlink = $this->makeKnownLink( $ns_user . ':' . $this->username, wfMsg( 'diff-to-lvr' ),
						$diff_oldid, '', '', '', wfMsg( 'diff-to-lvr_tooltiptext' ) );
					$newmsg_user = true;
				}
				switch (true) {
