...
class PageHistory {
	...
	function PageHistory($article) {
		...
		$this->mLastvisitedRevision= NULL;
	...
	function history() {
		...
		foreach (array_slice($revisions, 0, $limit) as $i => $line) {
			...
			$s .= $this->historyLine($line, $next, $counter, $this->getNotificationTimestamp(), $this->getLastvisitedRevision(), $first );
			$counter++;
		}
	...
	/** @todo document */
	function historyLine( $row, $next, $counter = '', $notificationtimestamp = false, $lastvisitedrevision = false, $latest = false ) {
		global $wgLang, $wgContLang;

		static $message;
		if( !isset( $message ) ) {
			foreach( explode( ' ', 'cur cur_tooltiptext last last_tooltiptext selectolderversionfordiff selectnewerversionfordiff  '.
						'updatedmarker updatedmarker_tooltiptext diff diff_tooltiptext hist hist_tooltiptext minoreditletter newpageletter blocklink undo '.
						'diff-to-lvr diff-to-lvr_tooltiptext lvr lvr_tooltiptext ' ) as $msg ) {
		...
		$curlink = $this->curLink( $row, $latest, $message[ 'cur_tooltiptext' ] );
		$lastlink = $this->lastLink( $row, $next, $counter, $message[ 'last_tooltiptext' ] );
		...
		if ($this->getNotificationTimestamp() && ($row->rev_timestamp >= $this->getNotificationTimestamp())) {
			$s .= $this->mSkin->makeKnownLinkObj( $this->mTitle, $message['updatedmarker'],
			  "diff=0&oldid=".$this->getLastvisitedRevision(), '', '', '', $message['updatedmarker_tooltiptext'] );
		}
		if( $row->rev_deleted ) {
	...
	/** @todo document */
	function curLink( $row, $latest, $tooltip ) {
		global $wgUser;
	...
	/** @todo document */
	function lastLink( $row, $next, $counter, $tooltip ) {
		global $wgUser;
	...
	function getLastvisitedRevision() {
		global $wgUser, $wgShowUpdatedMarker;

		if ($this->mLastvisitedRevision !== NULL)
			return $this->mLastvisitedRevision;

		if ($wgUser->getID() == 0 || !$wgShowUpdatedMarker)
			return $this->mLastvisitedRevision = false;

		$db =& wfGetDB(DB_SLAVE);

		$this->mLastvisitedRevision = $db->selectField(
			'watchlist',
			...
			"PageHistory::getLastvisitedRevision");
