...
class Linker {
	...
	function Linker() {
		...
		$this->mOptions['hover'] = $wgUser->getOption('hover');
	}
	...
	/** @todo document */
	function getInternalLinkAttributes( $link, $text, $broken = false, $hovertext= '' ) {
		$link = urldecode( $link );
	 ...
	 * @param bool $broken
	 */
	function getInternalLinkAttributesObj( &$nt, $text, $broken = false, $hovertext= '' ) {
		if( $broken == 'stub' ) {
			$r = ' class="stub"';
		} else if ( $broken == 'yes' ) {
			$r = ' class="new"';
		...
		if( $this->mOptions['hover'] ) {
			$r .= ' title="' . $hovertext . ' ' . $nt->getEscapedText() . '"';
		}
		return $r;
	}
	...
	/** @todo document */
	function makeKnownLink( $title, $text = '', $query = '', $trail = '', $prefix = '',$aprops = '', $hovertext = '') {
		$nt = Title::newFromText( $title );
		if ($nt) {
			return $this->makeKnownLinkObj( Title::newFromText( $title ), $text, $query, $trail, $prefix , $aprops, $hovertext );
		} else {
	 ...
	 * Pass a title object, not a title string
	 */
	function makeKnownLinkObj( $nt, $text = '', $query = '', $trail = '', $prefix = '' , $aprops = '', $hovertext= '' ) {
		global $wgTitle;
		...
		$style = $this->getInternalLinkAttributesObj( $nt, $text, '', $hovertext );
