...
class Parser
{
	...
	function parse( $text, &$title, $options, $linestart = true, $clearState = true ) {
		...
		global $wgUseEditor, $wgEditorToken;
                if ( isset($wgEditorToken) && eregi($wgEditorToken, $text, $eregi_result) ) {
			$wgUseEditor = true;
		}
		...
		# patch FCKEditor
		if ($wgUseEditor && isset($wgEditorToken) ) $text = preg_replace("|$eregi_result[0]|", "", $text);
...
'<head><title>test</title></head><body>'.$text.'</body></html>';
	...
	function internalParse( $text ) {
		...
		global $wgUseEditor; # patch editor

		if (!$wgUseEditor) $text = $this->doHeadings( $text );
		...
		if (!$wgUseEditor) $text = $this->doAllQuotes( $text );
		...
		if (!$wgUseEditor) $text = $this->doTableStuff( $text );
