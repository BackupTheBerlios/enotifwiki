...
END;
	...
	function pageTitleLinks() {
		...
		if ( $wgShowNewtalkForUserOrUserTalkPage ) {
			# do always show "You have new messages" text
			# checks are difficult and must consider many cases
				$username = $wgUser->getName();
				$userpage = $wgContLang->getNsText( Namespace::getUser() ) . ":" . $username;
				$usertitle = Title::newFromText( $userpage );
				...
				if ($wgUser->checkNotificationPendingForArticleOrTalk( $usertitle, NS_USER_TALK, $lvr )) {
					...
					$newmsg_usertalklink = $this->makeKnownLink( $ns_usertalk . ':' . $username, wfMsg( 'newmsg_usertalk' ) );
					$newmsg_usertalklvrlink = $this->makeKnownLink( $ns_usertalk . ':' . $username, wfMsg( 'diff-to-lvr' ),
						$diff_oldid, '', '', '', wfMsg( 'diff-to-lvr_tooltiptext' ) );
				...
				if ($wgUser->checkNotificationPendingForArticleOrTalk( $usertitle, NS_USER, $lvr )) {
					...
					$newmsg_userlink = $this->makeKnownLink( $ns_user . ':' . $username, wfMsg( 'newmsg_user' ) );
					$newmsg_userlvrlink = $this->makeKnownLink( $ns_user . ':' . $username, wfMsg( 'diff-to-lvr' ),
						$diff_oldid, '', '', '', wfMsg( 'diff-to-lvr_tooltiptext' ) );
					$newmsg_user = true;
				}
				switch (true) {
				case ( $newmsg_user &&  $newmsg_usertalk): {
					$str = $newmsg_userlink . ' (' . $newmsg_userlvrlink . ') ' . wfMsg('newmsg_and') . ' ' .
						$newmsg_usertalklink . ' (' . $newmsg_usertalklvrlink . ')';
					break;
					}
				case (!$newmsg_user &&  $newmsg_usertalk): {
				...
				default: { # this is the case for anon talk
					$str = $this->makeKnownLink( $wgContLang->getNsText( NS_USER_TALK ).':'.$this->username, wfMsg( 'newmsg_usertalk' ) );
					}
				}
				$s .= ' | <strong>'. wfMsg( 'newmsg', $str . '</strong>');

			} else { # old method, unchanged
	...
	function nameAndLogin() {
		...
		if ( $wgUser->isAnon() ) {
			...
			global $wgAutoLogin;
			if (!$wgAutoLogin) {
			 	$logoutLink =  $this->makeKnownLinkObj( Title::makeTitle( NS_SPECIAL, 'Userlogout' ), wfMsg( 'logout' ),
				"returnto={$rt}" ) . ' | ' ;
			} else $logoutLink = '';
...
#		       0 != $id->getEmailAuthenticationTimestamp() && # .. which is confirmed
