...
class PreferencesForm {
	...
	function PreferencesForm( &$request ) {
		...
		$this->mEnotifNewPages = $request->getCheck( 'wpEnotifNewPages' ) ? 1 : 0;
	...
	function savePreferences() {
		...
		global $wgEnotifNewPages,$wgEnotifNewPagesAllowedForAll;
		global $wgAuth;
		...
		if ( $wgEnotifNewPages &&
			( $wgEnotifNewPagesAllowedForAll || $wgUser->isBureaucratOrDeveloperOrSysop() ) ) {
			$wgUser->setEnotifNewPages( $this->mEnotifNewPages );
		} else {
			$wgUser->setEnotifNewPages( false );
		}
		...
		if( $wgEnableEmail ) {
			...
			if( ($newadr != '') && ($newadr != $oldadr) ) {
				...
				if( $wgUser->isValidEmailAddr( $newadr ) ) {
					...
					global $wgEmailAuthenticationRequiredForAll;
					if ( !$wgEmailAuthenticationRequiredForAll && $wgUser->isBureaucratOrDeveloperOrSysop() ) {
						$wgUser->mEmailAuthenticated = wfTimestampNow( TS_MW );
 						$wgUser->saveSettings();
					} else {
						$wgUser->mEmailAuthenticated = null; # but flag as "dirty" = unconfirmed
						$wgUser->saveSettings();
						if ($wgEmailAuthentication) {
							# Mail the confirmation link (token) to the unconfirmed address.
	...
	function resetPrefs() {
		...
		$this->mEnotifNewPages = $wgUser->getEnotifNewPages();
	...
	function mainPrefsForm( $status , $message = '' ) {
		...
		global $wgRCShowWatchingUsers, $wgRCShowCurrentRevisionOnly;
		global $wgEnotifRevealEditorAddress, $wgEnotifFromEditor;
		...
		if ($wgEmailAuthentication && ($this->mUserEmail != '') ) {
			if( $wgUser->getEmailAuthenticationTimestamp() ) {
				$emailconfirmed = wfMsg('emailauthenticated',$wgUser->getEmail(), $wgLang->timeanddate($wgUser->getEmailAuthenticationTimestamp(), true ) ).'<br />';
				$disabled = '';
			} else {
				$skin = $wgUser->getSkin();
				$emailconfirmed = wfMsg('emailnotauthenticated').'<br />' .
					$skin->makeKnownLinkObj( Title::makeTitle( NS_SPECIAL, 'Confirmemail' ),
						wfMsg( 'emailconfirmlink' ) );
				$disabled = ' '.wfMsg('disableduntilconfirmation');
			}
		} else {
			$emailconfirmed = '';
		...
		$enotifwatchlistpages = ($wgEnotifWatchlist) ? $this->getToggle( 'enotifwatchlistpages', $disabled ) : '';
		$enotifusertalkpages = ($wgEnotifUserTalk) ? $this->getToggle( 'enotifusertalkpages', $disabled ) : '';
		$enotifminoredits = ($wgEnotifWatchlist && $wgEnotifMinorEdits) ? $this->getToggle( 'enotifminoredits', $disabled ) : '';
		$enotifrevealaddr = (($wgEnotifWatchlist || $wgEnotifUserTalk) && $wgEnotifRevealEditorAddress && $wgEnotifFromEditor ) ? $this->getToggle( 'enotifrevealaddr', $disabled ) : '';

		if ( $wgEnotifNewPages &&
			( $wgEnotifNewPagesAllowedForAll || $wgUser->isBureaucratOrDeveloperOrSysop() ) ) {
			if ( $this->mEnotifNewPages ) { $enotifnewpagec = 'checked="checked"'; }
			else { $enotifnewpagec = ''; }
			$enotifnewpages = "<div><label><input type='checkbox' $enotifnewpagec value=\"1\" name=\"wpEnotifNewPages\" />".
						wfMsg( 'enotifnewpages' ).$disabled."</label></div>";
		} else {
			$enotifnewpages = '';
		}
		...
		$wgOut->addHTML( '<fieldset><legend>' . htmlspecialchars(wfMsg('prefs-rc')) . '</legend>
			...
			$this->getToggles( array(
				...
				'usenewrc',
				$wgRCShowCurrentRevisionOnly ? 'rccurrevonly' : false,
				$wgShowUpdatedMarker ? 	array('showupdated', wfMsg( 'updatedmarker' ) ) : false
				)
			) . '</fieldset>'
		);
