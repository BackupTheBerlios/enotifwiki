...
class LoginForm {
	...
	function processLogin() {
		...
		# auto-confirm e-mail address for super-users
#		global $wgEmailAuthenticationRequiredForAll,$wgEmailAuthentication;
#		if ( $wgEmailAuthentication
#			&& $u->isBureaucratOrDeveloperOrSysop()
#			&& !$wgEmailAuthenticationRequiredForAll
#			&& !$u->getEmailAuthenticationTimestamp() ) {
#			$wgUser->mEmailAuthenticated = wfTimestampNow( TS_MW );
#		}

		$wgUser->saveSettings();

#		if ( !$wgEmailAuthentication || $alreadyconfirmed ) {
#			$confirmed = '';
#			$mailmsg = '';
#		} elseif ($u->mEmailAuthenticationtimestamp != wfTimestampOrNull(0) ) {
#				$confirmed = ' ' . wfMsg( 'emailconfirmed', $u->getEmail(), $wgLang->timeanddate( $u->mEmailAuthenticationtimestamp, true ) );
#			} else {
#				$confirmed = ' ' . wfMsg( 'emailnotconfirmed' );
#			}
#		if ( $wgEmailAuthentication ) {
#			} elseif ( $u->getEmailAuthenticationTimestamp() ) {
#				$emailconfirmed = ' ' . wfMsg( 'emailconfirmed', $u->getEmail(), $wgLang->timeanddate( $u->getEmailAuthenticationTimestamp(), true ) );
#			} else {
...
#		}
	...
	function mailPasswordInternal( $u ) {
		...
		$m = wfMsg( 'passwordremindertext', $ip, $u->getName(), $np, wfUrlencode( $u->getName() ) );
